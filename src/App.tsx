        ...
        <div className="monitor-nav flex items-center gap-3 rounded-full px-4 py-2.5">
          ...
          {demo && (
            <button
              onClick={exitDemo}
              title="点击退出演示模式"
              className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium border border-white/45 bg-white/25 text-slate-800 dark:text-white/90 hover:bg-white/35 dark:border-white/25 dark:bg-white/10 dark:hover:bg-white/20 transition-all active:scale-[0.96] cursor-pointer shadow-xs backdrop-blur-md"
            >
          ...
          <Button variant="glass" size="icon" onClick={toggleTheme} title="切换外观主题">
            {dark ? <Sun className="size-4 text-amber-400" /> : <Moon className="size-4 text-slate-800 dark:text-white" />}
          </Button>
        </div>
      </header>
